...
> [!CAUTION]
> ChatGPT Mark 目前已暫停開發，且不再建議作為日常使用工具。
>
> 近期 ChatGPT 前端的大幅變動，尤其是虛擬化（virtualized）對話渲染機制，已使書籤跳轉功能變得不穩定。
>
> 較舊的訊息通常不會存在於目前頁面的 DOM 中，必須等 ChatGPT 動態重新渲染後才會出現，因此長距離跳轉可能會變得緩慢、失敗，或完全無法運作。
>
> 附近的書籤在某些情況下可能仍可正常使用，但目前已無法保證穩定的導航體驗。
>
> 專案目前已無限期停止大型功能開發，未來可能只會接受少量維護性修正。
>
> **抱歉，我可能已經死了!**
